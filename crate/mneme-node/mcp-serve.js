...
async function run() {
  ...
  server.registerTool(
    'icarus_ingest',
    {
      ...
      description: 'Use when the user points at a folder/file of documents (pdf/docx/pptx/images/audio/txt/md) and wants it searchable — NOT for a single fact or decision (use icarus_save_memory for that instead). Default is LOCAL .amr only (self-hosted). HIVEMIND cloud ingest is opt-in via ICARUS_LOCAL_ONLY=0 plus local=false. Text-only local formats: txt/md/json/csv/log.',
      inputSchema: {
