...
async function run() {
  ...
  server.registerTool(
    'icarus_recall',
    {
      ...
      description: 'Call FIRST before answering anything that might already be known — a past decision, a preference, a prior conversation, code context saved with icarus_save_memory/icarus_ingest_code, or evidence from icarus_ingest. Cheap, local, no reason not to check. LOCAL ONLY, always -- never routes to HIVEMIND\'s shared /api/recall (a real cross-tenant leak was found there in testing). Real parallel hybrid retrieval: dense (HNSW) and lexical (BM25) run concurrently, merged via Reciprocal Rank Fusion. If HIVEMIND is connected, the merged set gets a narrow re-score from the real bge-reranker-v2-m3 cross-encoder; if not, the hybrid merge itself is the final answer. Automatically excludes superseded memories (anything replaced via relationship:"update" or icarus_update_memory) -- only the latest version of a corrected fact surfaces. Note: there is deliberately no separate "AI-powered synthesis" recall tool here (unlike hivemind_query_with_ai) -- the calling agent already IS the reasoning layer; call this for raw grounded results, then reason over them directly.',
      inputSchema: {
