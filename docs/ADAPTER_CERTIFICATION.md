...
| Codex CLI | Compatible / experimental app-server bridge | The Rust bridge has a fake protocol conformance fixture. A local disposable-repository run against Codex CLI `0.149.0` performed one contract-allowed write and one forbidden `README.md` write attempt. The allowed write received structured Rust authorization and was the only reconciled file. The forbidden request received a structured decline; Codex then emitted the documented terminal `fileChange: declined` item, which Rust binds to that exact decline without treating it as a write. A separate live attempt to create an external marker produced `item/commandExecution/requestApproval`; Rust declined it, Codex emitted `commandExecution: declined`, the marker was absent, and reconciliation had zero changes. The authoritative `README.md` remained unchanged and both tasks entered `verifying`. A fresh Codex process has also resumed the persisted exact thread under a new linked ICARUS execution; Rust clears one-shot approvals before the resumed turn. | An external-write approval/revocation run and the remaining certification evidence. |
