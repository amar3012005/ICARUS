...
| Codex CLI | Compatible / experimental app-server bridge | The Rust bridge has a fake protocol conformance fixture. A local live probe against Codex CLI `0.134.0` completed `initialize`, `thread/start`, one managed `turn/start`, and native thread/turn/item/completion receipts. | The observed turn did not produce a structured file-change approval. ICARUS therefore cannot prove pre-action interception of a real Codex write, and cannot certify the client. |
