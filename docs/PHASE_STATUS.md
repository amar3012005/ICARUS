...
| Node-layer CI (CLI, TUI, MCP, installers, recall, ingestion, skills, binary smoke) | **PARTIAL** | Public CI runs on macOS and Linux. A native-addon MCP job now drives the actual stdio process through local lexical ingest, structured save, recall, and get-by-id against a disposable shard, with no credentials or network. The framing suite separately rejects non-JSON stdout. **Still missing:** independent compiled-binary smoke outside the release workflow, real PTY coverage, and broader upgrade/rollback coverage. |
