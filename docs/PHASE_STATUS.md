...
| 4 — managed launcher and adapters | **IMPLEMENTED, COMPATIBILITY ONLY** | Isolated worktrees by default, current-workspace acknowledgement/baselines, Rust scope reconciliation, wall-time deadlines, and Claude edit/stop hooks are covered. A native-addon fake Claude process now exercises the public CLI through task preparation, lifecycle receipts, pre/post write hooks, scope reconciliation, and verification handoff without a model/network dependency. Codex app-server handling now has a Rust-owned stable-thread binding plus bounded lifecycle capture and fail-closed command/file/permission approval decisions; the Node layer is a transparent native-call transport only. Its live app-server bridge and protocol conformance fixture are still unfinished, and the upstream file-change approval request has no individual paths, so Codex remains **compatible**, never certified. Claude is also not certified. |
