...
Last updated after the independently verified CLI release **0.3.60**. Newer hardening on `main`
...
| Automated release: asset naming, checksums, provenance, draft detection, cold download, version execution | **DONE for v0.3.60** | Public release `v0.3.60` passed version validation, Linux x64 + macOS ARM64/x64 builds, exact binary version execution, checksums, GitHub build-provenance attestation, non-draft publishing, expected asset checks, cold download/hash/execution, and `releases/latest` routing. A clean macOS ARM64 download was independently SHA-256 checked, executed as `0.3.60`, and had its GitHub attestation verified. |
...
| 2 — task contracts and lifecycle | **IMPLEMENTED, HARDENING IN PROGRESS** | Immutable contracts, amendments, approvals, checkpoints, transitions, resume divergence checks, and task-scoped context have Rust tests. Fresh-process resume now covers every non-terminal lifecycle state and verifies task identity, execution linkage, persisted state, and the event chain. Process-kill fault injection at each on-disk write boundary remains open. |
...
| 8 — production hardening / RC | **PARTIAL HARDENING** | NUL-path, rename, symlink-artifact, MCP-stdio, fresh-process lifecycle-resume, and a fake Claude-compatible end-to-end managed-launch/hook conformance regression are covered. Releases now include GitHub build-provenance attestations and update downloads are checksum-validated before replacement. It still requires Codex and real-client conformance, the public corpus, process-kill fault injection, installer rollback, SBOMs, client-side attestation/offline-signature verification, and release-candidate gates. |
