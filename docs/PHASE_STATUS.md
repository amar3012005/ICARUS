...
| 8 — production hardening / RC | **PARTIAL HARDENING** | NUL-path, rename, symlink-artifact, MCP-stdio, and fresh-process lifecycle-resume regressions are covered. It still requires the public corpus, full fake-agent conformance, process-kill fault injection, installer rollback, SBOM/provenance/signature, and release-candidate gates. |
