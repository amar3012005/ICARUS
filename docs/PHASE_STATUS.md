...
| 4 — managed launcher and adapters | **IMPLEMENTED, COMPATIBILITY ONLY** | Isolated worktrees by default, current-workspace acknowledgement/baselines, Rust scope reconciliation, wall-time deadlines, and Claude edit/stop hooks are covered. A native-addon fake Claude process now exercises the public CLI through task preparation, lifecycle receipts, pre/post write hooks, scope reconciliation, and verification handoff without a model/network dependency. Codex now has a Rust app-server bridge embedded in the native addon plus a CLI-to-addon fake-server conformance fixture covering initialize, native thread binding, turn start, fail-closed file approval, and lifecycle receipts; the CLI only invokes the native call. Release builds run that fixture after compiling each platform addon. `icarus run --agent codex --codex-app-server` opts into this experimental bridge, while the stable CLI path remains the default. Authentic live-client conformance is still open, and the upstream file-change approval request has no individual paths, so Codex remains **compatible**, never certified. Claude is also not certified. |
