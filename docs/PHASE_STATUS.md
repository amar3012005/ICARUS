...
| 8 — production hardening / RC | **PARTIAL HARDENING** | NUL-path, rename, symlink-artifact, MCP-stdio, fresh-process lifecycle-resume, and a fake Claude-compatible end-to-end managed-launch/hook conformance regression are covered. It still requires Codex and real-client conformance, the public corpus, process-kill fault injection, installer rollback, SBOM/provenance/signature, and release-candidate gates. |
