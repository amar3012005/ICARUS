...
| 8 — production hardening / RC | **PARTIAL HARDENING** | NUL-path, rename, symlink-artifact, MCP-stdio, fresh-process lifecycle-resume, event-append process-abort recovery, task-snapshot atomic-replacement recovery, the all-public-v0.3 migration corpus, and a fake Claude-compatible end-to-end managed-launch/hook conformance regression are covered. Releases now include GitHub build-provenance and SPDX SBOM attestations, and update downloads are checksum-validated before replacement. v0.3.62 additionally retains one rollback binary in `install.sh`, restores an interrupted handoff, and regression-tests final-move failure in an isolated home; its ARM64 public binary was independently verified after release. v0.3.63 process-tests the Rust atomic snapshot path after candidate fsync+rename and before directory sync, proving a complete readable/rewriteable JSON value after process death (not physical-power-loss emulation); the task lifecycle now receives the same targeted process-death coverage. The local Codex 0.134.0 app-server has completed the real stdio initialize handshake; authentic managed-turn conformance is still open. It still requires Codex and real-client conformance, broader process-kill coverage, client-side attestation/offline-signature verification, and release-candidate gates. |
